...
namespace Examen.App.Comun
{
    public class AutoRelayCommand : RelayCommand, IDisposable
    {
        ...
        private string GetPropertyName<T>(Expression<Func<T>> propertyExpression)
        {
            ...
            if (body == null)
            {
                throw new ArgumentException("Argumento invalido", nameof(propertyExpression));
            }

            var property = body.Member as PropertyInfo;
            if (property == null)
            {
                throw new ArgumentException("El argumento no es una propiedad", nameof(propertyExpression));
            }
